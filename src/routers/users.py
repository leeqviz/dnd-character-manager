...
async def get_user(user_id: uuid.UUID, users_service: UsersService = Depends(get_users_service)):
...
async def update_user(user_id: uuid.UUID, user: UserIn, users_service: UsersService = Depends(get_users_service)):
    try:
...
async def delete_user(user_id: uuid.UUID, users_service: UsersService = Depends(get_users_service)):
    try:
